...
class TransferTreeRegressor(BaseAdaptEstimator):
    """
    TransferTreeRegressor
    
    Decision tree regressor structure for model-based transfer algorithms.
...
"Transfer Learning on Decision Tree with Class Imbalance". In IEEE ICTAI, 2019.
    ...
    def _modify_tree(self, dtree, X, y,**fit_params):
        ...
        if self.algo == "src" or self.algo == "source":
            ...
            return self._retrain(X,y,**fit_params)
        elif self.algo == "" or self.algo == "relabel":
            return self._relab(X, y,**fit_params)
        elif self.algo == "ser":
            return self._ser(X, y,**fit_params)
    ...
    def _relab(self, X_target_node, Y_target_node, node=0):
        ...
        current_class_distribution = np.mean(Y_target_node,axis=0)
        self.updateValue(node,current_class_distribution)
    ...
    def _ser(self,X_target_node,y_target_node,node=0,original_ser=True,
        ...
        val = np.zeros((self.estimator_.n_outputs_, 1))

        # for i in range(self.estimator_.n_classes_):
        #     val[:, i] = list(y_target_node).count(i)
        val[:,0] = np.mean(y_target_node,axis=0)
        ...
        if self.estimator_.tree_.feature[node]== -2:
            ...
            if original_ser:
                if y_target_node.size > 0 and len(set(list(y_target_node))) > 1:
                    ...
                    if max_depth is not None:
                        d = self.depths[node]
                        DT_to_add = DecisionTreeRegressor(max_depth = max_depth - d + 1)
                        
                    else:
                        DT_to_add = DecisionTreeRegressor()
...
#        Tree_ = self.estimator_.tree_
        ...
        #current_class_distribution = ut.compute_class_distribution(classes_, Y_target_node)
        current_class_distribution = np.array(list(set(Y_target_node)))
        ...
        Q_source_parent = ut.get_node_distribution(self.estimator_,node)
        # A changer : toutes les fonctions dans utils relatives aux classes.
        # Le plus simple : remplacer à la fois le DG et le IG par la MSE (estimateur de variance) 
        # --> possible d'étendre la notion de pruning risk en utilisant l'inégalité de chebychev
...
class TransferTreeSelector(BaseAdaptEstimator):
    ...
    def fit(self, Xt=None, yt=None, **fit_params):
        ...
        for k,algo in enumerate(self.algorithms):
            ...
            #self.transferred_models[k].fit(Xt=Xt, yt=yt,root_source_values=self.root_source_values,coeffs=self.coeffs,**kwargs,**fit_params)
...
class TransferForestRegressor(BaseAdaptEstimator):
    """
    TransferForestRegressor
    
    Random forest regressor structure for model-based transfer algorithms.
    ...
    Parameters
    ----------    
    estimator : sklearn RandomForestRegressor (default=None)
    ...
    Attributes
    ----------
    estimator_ : sklearn RandomForestRegressor
        Transferred random forest regressor using target data.

    rf_size : int.
        
    estimators_ : numpy array of TransferTreeRegressor.
...
"Transfer Learning on Decision Tree with Class Imbalance". In IEEE ICTAI, 2019.
    ...
    def __init__(self,
        ...
        if not isinstance(estimator, RandomForestRegressor):
            raise ValueError("`estimator` argument must be a ``RandomForestRegressor`` instance, got %s."%str(type(estimator)))
        ...
        for i in range(self.rf_size):
            self.estimators_[i] = TransferTreeRegressor(estimator = self.estimator_.estimators_[i], algo = self.algo)
...
class TransferForestSelector(BaseAdaptEstimator):
    ...
    def model_selection(self, Xt=None, yt=None, score_type = "auc", oob_ = False, **fit_params):
        ...
        for k in range(self.rf_size):
            TTS = TransferTreeSelector(estimator=self.STRF_model.estimator_.estimators_[k],algorithms=self.algorithms,list_alg_args=self.list_alg_args)
            #TTS = TransferTreeSelector(estimator=self.estimator_.estimators_[k],algorithms=self.algorithms, list_alg_args=self.list_alg_args,
    ...
    #     inds, oob_inds = ut._bootstrap_(yt.size,class_wise=True,y=yt)
    #     for k,algo in enumerate(self.algorithms):
    #         self.transferred_models[k].fit(Xt=Xt[inds], yt=yt[inds],**fit_params)
